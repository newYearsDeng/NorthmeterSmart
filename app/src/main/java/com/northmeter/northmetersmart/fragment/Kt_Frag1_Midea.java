 ...
 */
@SuppressLint("NewApi")
public class Kt_Frag1_Midea extends Fragment implements OnClickListener {
	private View view;
	private int temputer=26;
	private ImageView model_change,speed_change,open_down,img_Refresh,air_baifeng,air_up_down,img_send;
	...
	private Runnable thread,runnable_1,runnable_2,runnable_3,runnable_4;;
	...
	private ImageView image0_air;//通讯质量显示
	private TextView textview2_air;//通讯网络字样
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		try{
			view = inflater.inflate(R.layout.f_yaokong_midea, container, false);
	...
	Handler handler = new Handler(){
		...
		try{
			...
			if(thz[0].equals("FSHWBM")&&thz[1].equals("SUCCESS")){	
				SharedPreferences sp=getActivity().getSharedPreferences("LastHwZl_midea", 0);
				Editor editor=sp.edit();
				editor.putString(str_mac, txt_temp.getText()+","+model_id+","+speed_id);
				editor.commit();
...
//						refashTHZZT();
					initDataView();
					progressDialog_1.dismiss();
					Toast toast=Toast.makeText(getActivity(),thz[1].split("/")[3], Toast.LENGTH_SHORT);
					toast.setGravity(Gravity.CENTER, 0, 0);
					toast.show();
					return;
				}
		...
		new Thread(){
			public void run(){
				try{
					...
					msg.setData(data);	
					Kt_Frag1_Midea.this.handler.sendMessage(msg) ;
					e.printStackTrace();
					}
				    }
				}.start();
	...
	public void init_first(){
		...
		air_baifeng = (ImageView) view.findViewById(R.id.air_baifeng);//摆风
		air_baifeng.setOnClickListener(this);
		...
		SharedPreferences sp=getActivity().getSharedPreferences("LastHwZl_midea", 0);
		String lastZl = sp.getString(str_mac, 26+","+0+","+0);
	...
	public void onClick(View v) {
		...
		case R.id.img_onoff://开关
			...
			SharedPreferences spa=getActivity().getSharedPreferences("LastHwZl_midea", 0);
			Editor Editor=spa.edit();
			Editor.putString(str_mac, txt_temp.getText()+","+model_id+","+speed_id);
			Editor.commit();

			String hwm_zl_1=kaiguan_2+","+model[model_id]+","+temputer+","+speed[speed_id];
			hwa_1 =MiderKTHW.getMiderKthw(hwm_zl_1);
...
//			air_up_down.setImageResource(getUpDownResource(updown));
		...
		case R.id.air_baifeng://摆风按钮
			...
			String hwm_zl_baifeng=kaiguan_1+","+"摆风";
			hwa_1 =MiderKTHW.getMiderKthw(hwm_zl_baifeng);
			System.out.println("hwa_1: "+hwa_1);
