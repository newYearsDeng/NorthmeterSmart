 ...
 * 用户用电报表数据曲线和温度数据曲线
 * */
public class Kt_Frag_Chart_2 extends Fragment implements OnClickListener,OnChartValueSelectedListener {
...
//					String rec_str = spf.getString(str_mac, "0000-00-00 00:00:00 0000-00-00 00:00:00 0 #000-00-00 00:00:00 000-00-00 00:00:00 0");
	...
	private void init_view(String[] dn_list,String[] time_list){
	   ...
	   // y.set(5); // y轴上的标签的显示的个数
		...
		if (data != null) {
			...
			LineDataSet set = new LineDataSet(yVals, "电能(kWh)");
	...
	private void init_temputer(String[] temp_list,String[] time_list){
		...
		//mYLabels_tem.setDrawYValgetYLabels(//设置是否显示y轴的值的数据
		
		YAxis y = mChart_tem.getAxisLeft(); // y轴的标示
	    y.setTextColor(mColors[3]);//	    y.setLabegetXLabels() // y轴上的标签的显示的个数
	...
	public void RegisterBroad(){
		...
		new Thread(){
			public void run(){
				...
				IntentFilter intentfilter = new IntentFilter("Intent.MessageAty_Read48h");
	...
	class ReceiveTool extends BroadcastReceiver{//selected/(注：前面这些数据不传递过来)  0a0001aa7k/002014110119/read48h/data
		...
		@Override
		public void onReceive(Context context, Intent intent) {//selected/    0a0001aa7k/002014110119/read48h/data
...
//										editor.commit();
										...
										msg.setData(data);	
										Kt_Frag_Chart_2.this.handler.sendMessage(msg);
									}
...
//	@Override
//	public void setDataChange(String msg) {//selected/(注：前面这些数据不传递过来)     0a0001a820/160127001282/read48h/data
//		// TODO Auto-generated method stub
//		final String receive = msg.split("/")[3];
//		System.out.println("收到收到收到收到收到到到到到：：2：++++++++++++++++：："+msg);
