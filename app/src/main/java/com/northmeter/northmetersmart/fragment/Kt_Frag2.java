...
import com.northmeter.northmetersmart.adp.F_Kt_Lt_adp;
...
/** 空调定时页面 */
public class Kt_Frag2 extends BaseActivity implements OnClickListener,OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try{
			...
			findViewById(R.id.img_sdcscsh).setOnClickListener(this);//初始化日程表
			...
			handler=new Handler(){
				public void handleMessage(Message mesg){
					...
					switch(result){
					case "delete_fail":
						Toast.makeText(Kt_Frag2.this, "删除失败", Toast.LENGTH_SHORT).show();
						break;
					case "delete_success":
						Toast.makeText(Kt_Frag2.this, "删除成功", Toast.LENGTH_SHORT).show();
						adp.notifyDataSetChanged();
						break;
					case "correct_Rcb_fail":
						Toast.makeText(Kt_Frag2.this, "日程表初始化失败", Toast.LENGTH_SHORT).show();
						break;
					case "correct_Rcb_success":
						Toast.makeText(Kt_Frag2.this, "日程表初始化完成", Toast.LENGTH_SHORT).show();
						break;
					}
	...
	private void init_view(boolean visibility){
		try{
			models.clear();
			DBInfraredCode dbinfrared=new DBInfraredCode(Kt_Frag2.this);
			...
			if(infrared!=null){
				for(int i=0;i<infrared.size();i++){
					...
					model.setTxt1(infrared.get(i).getSwitchs());
			...
			adp = new F_Kt_Lt_adp(Kt_Frag2.this,models);
			adp.notifyDataSetChanged();
...
//	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.relativeLayout2:
				Intent intent_1 = new Intent(Kt_Frag2.this,Kt_Frag3.class);
			...
			case R.id.ima_remove:
				...
				if(flag==false){
					Toast.makeText(Kt_Frag2.this, "未选择任何任务删除！", Toast.LENGTH_SHORT).show();
					return;
				}
				progressDialog=CustomProgressDialog.createDialog(Kt_Frag2.this);
							...
							for (int i = 0; i < modelsToDelete.size(); i++){
								System.out.println("设备总数量=" + modelsToDelete.get(i).getId());
									...
									if(flag==false){
										...
										mesg.setData(data);
										Kt_Frag2.this.handler.sendMessage(mesg);
										break;
									}else{
										...
										String task_str = "delCycleTask/"+str_mac+"#"+fs_code+"&"+str_mac+"&"+modelsToDelete.get(i).getTime()+"&"+"0a0001aa7k"+"&"
										+modelsToDelete.get(i).getTxt1()+"&"+modelsToDelete.get(i).getTxt3()+
												"&"+modelsToDelete.get(i).getTxt2()+"&"+modelsToDelete.get(i).getTxt4()+"&"+modelsToDelete.get(i).getTxt5()
												+"&"+modelsToDelete.get(i).getTxt6()+"&"+modelsToDelete.get(i).getWeekday();
										delete_Task_Data(task_str);
										
										//从数据库里删除选中的设备
										//dbinfrared.Delete(modelsToDelete.get(i).getId(), str_mac);
				...
				public void run(){
					...
					if(rst_raw!=null){
						...
						if(rst_error.equals("200")){
							...
							msg.setData(data);		
							Kt_Frag2.this.handler.sendMessage(msg) ;
							}else{
								Message msg=new Message();
								Bundle data=new Bundle();
	...
	/**删除以设置好的定时任务到服务器*/
	private void delete_Task_Data(final String data){//delCycleTask/str_mac#3（红外码编号）&002014110119&08:15&0003（空调类型）&开&制冷（模式）&24（温度）&三级（风速）&上下（上下扫风开）&左右（左右扫风开）&[一, 二, 三, 四, 五]
		new Thread(){
			public void run(){
				try {
					PublishMqttMessageAndReceive.getInstance().PublishMessage(getApplicationContext(),
		...
		new Thread(){
			public void run(){
				try {
					...
					PublishMqttMessageAndReceive.getInstance().PublishMessage(getApplicationContext(),
							"0a0001aa7k/"+str_mac,"tasks_/read");	
					//PublishMqttAddMRecord pushMR = new PublishMqttAddMRecord("0a0001aa7k/"+str_mac,"tasks_/read");
				} catch (MqttException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			if(rst_raw_1!=null){
				...
				if(rst_1!=null){ 
					...
					MyOrder odToSend=OrderList.Send_Zigbee_ByDeviceType(str_type, 
							str_mac, OrderList.KEEP_ZIGBEE_WEEK_TASK,"","",rcb+rst_1);	
