...
/**zigbee四路灯控*/
public class Add_Fragment4_6 extends Fragment implements OnClickListener{
	private View view;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.f_add_4_6, container, false);
		view.findViewById(R.id.button_next4_6).setOnClickListener(this);
...
//				progressDialog.dismiss();
				...
				if(mqttreceive[0] == "MQTT"){
					...
					case "MQTTerr":
						...
						.setChecked(true);
	                	FragmentHelper.loadFragment(FragmentHelper.fragments.get(15));//通过Fragment设置选
	                	return;
					case "TIMEOUT":
						progressDialog.dismiss();
...
//					Add_Fragment4_6.this.handler.sendMessage(message);
//					e1.printStackTrace();
//				}
			}
		}.start();
		...
		public String sendMsgByUserCommand(int order_type,String cmd_data){
			...
			String rst_raw = OrderManager.getInstance().sendOrder(
					OrderList.getSendByDeviceType(
							Type_Entity.Four_street_control,
				...
				if(stay>=0){
					try {
						PublishMqttMessageAndReceive.getInstance().PublishMessage(getActivity(),
								"addmanagerecord;managerecord","0a0001a830/"+
										DeviceManager.getInstance().getTempDevice().getMac()+"/wallsocket/0a0003ahup/"+
										DeviceManager.getInstance().getTempDevice().getContact()+"/ethnet");
...
//								DeviceManager.getInstance().getTempDevice().getContact()+"/ethnet");
					...
					msg.setData(data);		
					Add_Fragment4_6.this.handler.sendMessage(msg);
				}else{
