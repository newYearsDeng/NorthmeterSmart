...
public class HandleMqttMessage {
	...
	private SharedPreferences spf,cp_state,sp_48h,sp_48h_socket;
	private static Editor edit,edit_cp,edit_48h,edit_48h_socket;
	public HandleMqttMessage(Context context){
		...
		this.spf = context.getSharedPreferences("status", context.MODE_PRIVATE);
		this.cp_state = context.getSharedPreferences("cp_state", context.MODE_PRIVATE);//通讯质量
		this.sp_48h = context.getSharedPreferences("form_48h", context.MODE_PRIVATE);//48小时用电曲线
		this.sp_48h_socket = context.getSharedPreferences("form_48h_socket", context.MODE_PRIVATE);
		edit=spf.edit();
		edit_cp = cp_state.edit();
		edit_48h = sp_48h.edit();
		edit_48h_socket = sp_48h_socket.edit();
	}
...
//						dbDiagram.Insert(diagram_data);
						...
						// 表号/标示码（UPDATA）
						getreceive = new String[]{"DEVICE_UPDATA",receive_last.split("#")[0]+"/"+receive_last.split("#")[1]+"/CENTER_UPDATA/"+msg};
			...
			case 7://获取报表数据
				...
				//selected/00010002/get_building_subentry_used_ele_of_month/2017-02 {"ac":448.4599999999998,"socket":26.85,"lighting":227.06} 2017-06-13 09:51:19
				if(receive.indexOf("get_building_used_ele_of_month")>=0 || receive.indexOf("get_building_subentry_used_ele_of_month")>=0){
					getreceive = new String[]{"SELETC",receive.split("/")[0]+"/"+receive.split("/")[1]+"/"+receive.split("/")[2]+"/"+receive.split("/")[3]};
					break;
				}
				...
				case "read30point"://selected/0a0001aa7k/002014110119/read30point/data
					...
					break;
				case "read48h":
					if(receive.split("/")[1].equals(Type_Entity.Socket_type) || receive.split("/")[1].equals(Type_Entity.Four_street_control)){
						edit_48h_socket.putString(receive.split("/")[2],receive.split("/")[4]);
						edit_48h_socket.commit();
					}else{
						edit_48h.putString(receive.split("/")[2],receive.split("/")[4]);
						edit_48h.commit();
					}					
					getreceive = new String[]{"SELETC",receive.split("/")[1]+"/"+receive.split("/")[2]+"/"+receive.split("/")[3]+"/"+receive.split("/")[4]};
					break;
				...
				case "get_meter_current_blob":// 中央空调实时数据块  selected/0a0001aa7k/002014110119/get_meter_current_blob/data
					getreceive = new String[]{"SELECT_CENTER",receive.split("/")[1]+"/"+receive.split("/")[2]+"/get_meter_current_blob_Success/"+receive.split("/")[4]};
					break;																
				...
				case "get_cq_of_meter"://查询单个设备通讯质量 selected/0a0001aa7k/002014110119/get_cq_of_meter/data
					edit_cp.putString(receive.split("/")[2],receive.split("/")[4]);
					edit_cp.commit();
					getreceive = new String[]{"SELECT_cq",receive.split("/")[1]+"/"+receive.split("/")[2]+"/"+receive.split("/")[3]+"/"+receive.split("/")[4]};
					break;
		...
		if(!devices.isEmpty() && devices.size()!=0){
			for(int i=0;i<devices.size();i++){
				...
				case MyDevice.DEVICE_ZIGBEE_SMART_GATEWAY://zigbee网关
					...
					break;
				case MyDevice.DEVICE_ZIGBEE_SMART_AIR://zigbee分体空调控制器
				...
				if(!msg.equals("") & status.equals("200")){
					...
					edit.commit();
							try{
								...
								edit.commit();
								getreceive = new String[]{"Zigbee_dwcssjk",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/Zigbee_dwcssjk/FAIL/"+msg};
							}
						getreceive = new String[]{"Zigbee_dwcssjk",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/Zigbee_dwcssjk/"+msg};
					}else{
						edit.putString(receive_msg.split("/")[1],status);
						edit.commit();
						getreceive = new String[]{"Zigbee_dwcssjk",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/Zigbee_dwcssjk/FAIL/"+msg};
					}				
				break;
				
			/** 0a0001a4r5/150907000125/dsj/dnsjk/{"order_no":"1704071009387F0FDC575D93C1E44C6D790C29B359","result":"
			 * 161.41,0.0000,26.0,01,2200,03,00014401,00010101,00043750,49612.25,000000,1704071009","status":"200","msg":"任务执行完成"}
			 * &sign=dc330242c748c8305d7a7aecff9b7d7e 2017-04-07 10:09:44*/
			case "dnsjk":
				final String msg_1 = OrderManager.getInstance().getItemByOrder(receive_msg,
						"result", -1);
				if(!msg_1.equals("") & status.equals("200")){
					edit.putString(receive_msg.split("/")[1],"200");
					edit.commit();
					getreceive = new String[]{"Zigbee_dwcssjk",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/Zigbee_dwcssjk/"+msg_1};
				}else{
					edit.putString(receive_msg.split("/")[1],status);
					edit.commit();
					getreceive = new String[]{"Zigbee_dwcssjk",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/Zigbee_dwcssjk/FAIL/"+msg_1};
				}				
				break;
			case "zdsb2"://0a0001a880/131701090026/dsj/zdsb2/{"order_no":"1707271452556692BA8E51FC5D8C0FB1E866FDC303","status":"200","result":"0600080009003000483100ffff5009ffffffffffff5214270717","msg":"任务执行完成","sign":"FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF","eqpt_type":"0a0001a880","eqpt_id_code":"131701090026","cmd_type":"dsj","cmd_id":"zdsb2","cmd_data":""}
				final String msg_2 = OrderManager.getInstance().getItemByOrder(receive_msg,
						"result", -1);
				if(!msg_2.equals("") & status.equals("200")){
					edit.putString(receive_msg.split("/")[1],"200");
					edit.commit();
					getreceive = new String[]{"zigbee_zdsb2",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/zigbee_zdsb2/"+msg_2};
				}else{
					edit.putString(receive_msg.split("/")[1],status);
					edit.commit();
					getreceive = new String[]{"zigbee_zdsb2",receive_msg.split("/")[0]+"/"+receive_msg.split("/")[1]+"/zigbee_zdsb2/FAIL"+msg_2};
				}	
				break;
